...
% Considero que actual[0] y actual[1] es una notación bastante liosa, aunque tampoco se me ocurre cómo mejorarla. Lo mismo ocurre con con_nuevo y sin_nuevo.
% Óscar
\begin{algorithm}[H]
	\begin{algorithmic}[1]
		\REQUIRE \ \\
	...
	\STATE{\texttt{max\_beneficio = 0}}\\\
     	\STATE{Creamos una cola \texttt{posibles\_mochilas}}
     	\STATE{\texttt{posibles\_mochilas.push([false,\dots,false])}}
     	
     	\WHILE{\texttt{posibles\_mochilas}$\neq \emptyset$}
	  \STATE{\texttt{actual = posibles\_mochilas.pop()}}
	  \IF{\texttt{\#actual == $n$}}
	    \IF{beneficio(\texttt{actual}) >\ \texttt{max\_beneficio}}
	      \STATE{\texttt{max\_beneficio} = beneficio(\texttt{actual})}
	      \STATE{\texttt{solución = actual}}
	    \ENDIF
	  \ELSE
	    \STATE{\texttt{con\_nuevo = sin\_nuevo = actual}
	    %\STATE{con\_nuevo[1] += beneficios[\#actual[0]]}
	    % ¿No sería con\_nuevo[0].push\_back(true)?
...
En el algoritmo, \textit{beneficio} es una función que calcula la suma de beneficios de un vector de booleanos $\delta$, $\sum_{i=1}^{n}b_i\delta_i$; y \textit{peso} halla la suma de pesos del vector $\delta$, $\sum_{i=1}^{n}\omega_i\delta_i$.
...
\begin{algorithm}[H]
	...
	\STATE{\texttt{max\_valor = 0}}
     	\STATE{Creamos una cola con prioridad \texttt{posibles\_mochilas}}
	  ...
	  \ELSE
	    \STATE{\texttt{con\_nuevo = sin\_nuevo = actual \\
	    con\_nuevo[1] += beneficios[\#actual[0]] \\
	    % ¿No sería con\_nuevo[0].push\_back(true)?
	    con\_nuevo.push\_back(true)\\
	    % Lo mismo
	    sin\_nuevo.push\_back(false)}}
	    \STATE{\texttt{nuevo\_peso = 'peso de' \texttt{con\_nuevo}}}
	    \IF{\texttt{nuevo\_peso <= limite}}
	      \STATE{\texttt{posibles\_mochilas.push(con\_nuevo)}}
	    \ENDIF
	    % No sé expresarlo mejor sin "parafrasear" código
	    \STATE{Calculamos el máximo beneficio a partir de los elementos que ya tenemos en la mochila}
	    \IF{\texttt{max\_valor < 'peso de' sin\_nuevo + máximo beneficio}}
	      \STATE{\texttt{posibles\_mochilas.push(sin\_nuevo)}}
	    \ENDIF
	  \ENDIF
     	\ENDWHILE
...
\section{Traveling Salesman Problem}
  ...
  \begin{algorithm}[H]
  	\begin{algorithmic}[1]
		...
		mejor\_ruta = empty Ruta}}
        \IF{\texttt{\#ruta == \#ciudades}}
          \STATE{\texttt{coste\_actual += distancia(ruta.ultima\_ciudad, ruta.primera\_ciudad)}}
	      ...
	      \ENDIF
	    \ELSE
	      \FORALL{\texttt{i} = indice hasta \texttt{\#ciudades}:}
